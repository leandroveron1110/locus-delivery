...
export const EditZoneForm = ({
  ...
  const handleDelete = () => {
    if (!initialData.id) return;
    if (confirm(`¿Estás seguro que quieres eliminar la zona "${initialData.name}"?`)) {
  ...
  return (
    <div className="flex gap-3 mt-6 overflow-x-auto flex-nowrap flex flex-col h-full bg-white rounded-3xl shadow-xl border border-gray-200 p-6 w-full max-w-sm lg:max-w-none">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 flex-1">
        ...
        <div className="flex flex-col">
          <label htmlFor="name" className="text-sm font-medium text-gray-700 uppercase tracking-wide mb-1">
            Nombre de la Zona
          </label>
          <input
            id="name"
        ...
        {/* Precio */}
        <div className="flex flex-col">
          <label htmlFor="price" className="text-sm font-medium text-gray-700 uppercase tracking-wide mb-1">
            Precio (ARS)
          </label>
          <input
            id="price"
        ...
        <div className="flex items-center">
          <input id="isActive" type="checkbox" {...register("isActive")} className="w-4 h-4 text-green-600 rounded" disabled={isLoading} />
        ...
        <fieldset className="p-4 rounded-xl border border-dashed border-gray-300">
          <legend className="flex items-center text-sm font-medium text-gray-700 mb-3 px-2">
            <Clock className="w-4 h-4 mr-2 text-gray-500" /> Horario de trabajo
          </legend>

          <div className="flex items-center mb-4">
            <input id="hasTimeLimit" type="checkbox" {...register("hasTimeLimit")} className="w-4 h-4 text-blue-600 rounded" disabled={isLoading} />
            <label htmlFor="hasTimeLimit" className="ml-2 text-sm text-gray-700 font-medium">
          ...
          {hasTimeLimit && (
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col">
                <label htmlFor="startTime" className="text-xs font-medium text-gray-500">Hora de inicio</label>
                <input
                  id="startTime"
                  type="time"
                  {...register("startTime", { required: hasTimeLimit ? "Hora de inicio requerida." : false })}
                  className="px-4 py-2 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-gray-800 shadow-sm transition duration-150"
                  disabled={isLoading}
                />
              ...
              <div className="flex flex-col">
                <label htmlFor="endTime" className="text-xs font-medium text-gray-500">Hora de fin</label>
                <input
                  id="endTime"
                  type="time"
                  {...register("endTime", { required: hasTimeLimit ? "Hora de fin requerida." : false })}
                  className="px-4 py-2 rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-gray-800 shadow-sm transition duration-150"
                  disabled={isLoading}
                />
        ...
        <div className="flex flex-wrap gap-3 mt-6 justify-center">
          <button
            type="submit"
            disabled={isLoading}
            className="flex-auto max-w-[200px] bg-blue-600 text-white py-2 px-5 rounded-xl shadow-md hover:bg-blue-700 transition disabled:bg-gray-400"
          >
          ...
          <button
            type="button"
            onClick={onCancel}
            className="flex-auto max-w-[200px] border border-gray-300 py-2 px-5 rounded-xl hover:bg-gray-100 transition text-gray-700"
          >
            Cancelar
          </button>
          <button
            ...
            className="flex-auto max-w-[200px] bg-red-600 text-white py-2 px-5 rounded-xl shadow-md hover:bg-red-700 transition"
          >
            Eliminar
          </button>
        </div>
      ...
      <div className="mt-4 p-4 bg-gray-50 rounded-xl text-sm text-gray-600 shadow-inner">
        ...
        <p>Número de vértices: <span className="font-mono">{vertexCount}</span></p>
